<script lang="ts">
	...
	async function animateBall(path: number[]) {
		...
		for (let i = 0; i < path.length; i++) {
			const bounce = path[i];
			if (bounce === 0) leftCount++;
			else rightCount++;

			const step = PEG_GAP_X / 2 / (i < 4 ? 1 : 1.6);
		...
		<div class="grid grid-cols-1 gap-8 md:grid-cols-2">
			<div class="flex flex-col space-y-4">
				...
				<div class="flex-1 overflow-x-auto">
					<div
						class="plinko-board relative mx-auto"
						style="width: {boardWidth(rows)}px; height: {TOP_PADDING + (rows + 1) * PEG_GAP_Y}px;"
						bind:this={boardEl}
					>
						{#each Array(rows) as _, row}
							{#each Array(row + 2) as _, col}
								...
								<div
									class="peg"
									style="left: {offset + col * PEG_GAP_X}px; top: {TOP_PADDING + row * PEG_GAP_Y}px;"
								></div>
							{/each}
						{/each}
