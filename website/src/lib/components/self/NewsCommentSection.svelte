<script lang="ts">
	...
	import type { NewsComment } from '$lib/types/news-comment';

	const { articleId }: { articleId: number } = $props();

	// Same room-naming convention used by the reaction bar and the
	// server's redis publish target (`comments:news:{id}`) — the generic
	// websocket bridge treats this exactly like a coin room, no bridge
	// changes required.
	const room = $derived(`news:${articleId}`);

	let comments = $state<NewsComment[]>([]);
	...
	// --- @mention autocomplete (identical behavior to coin comments) ---
	...
	async function loadComments() {
		...
		try {
			...
			console.error('Failed to load news comments:', e);
		} finally {
			isLoading = false;
		}
	}
	...
	async function toggleLike(commentId: number) {
		...
		fetch(`/api/news/${articleId}/comments/${commentId}/like`, {
	...
	$effect(() => {
		// Re-run when articleId changes (e.g. client-side nav between two
		// article pages) so comments always reflect the current article.
		void articleId;
		loadComments();
	});
</script>
									...
									<HoverCard.Root>
										...
										>
											<span class="block truncate"
												><UserName name={comment.userName} nameColor={comment.userNameColor}
												/></span
											>
										</HoverCard.Trigger>
										...
										{#each parseMentions(displayContent) as part}
											{#if part.type === 'mention'}
												<a
													...
													class="text-primary hover:underline font-medium">@{part.username}</a
												>
											{:else}
												{part.value}
											{/if}
