<script lang="ts">
	...
	let recipientIsFriend = $state(false);
	...
	// Pre-send estimate. Friends transfer cash fee-free (enforced server-side in
	// /api/transfer) - recipientIsFriend is looked up below whenever the recipient
	// username changes, so this preview matches what the server will actually charge.
	// The actual amount received always comes from the server response regardless
	// (see amountReceived in handleSend below), so this is a cosmetic-only lookup.
	let cashTransferFee = $derived(
		transferType === 'CASH' && !recipientIsFriend ? numericAmount * CASH_TRANSFER_FEE_RATE : 0
	);
	...
	let recipientCheckTimeout: ReturnType<typeof setTimeout> | undefined;

	async function checkRecipientFriendship(username: string) {
		if (!username.trim()) {
			recipientIsFriend = false;
			return;
		}
		try {
			...
			if (res.ok) {
				...
				recipientIsFriend = data?.profile?.viewerFriendshipStatus === 'friends';
			} else {
				recipientIsFriend = false;
			}
		} catch {
	...
	$effect(() => {
		...
		recipientCheckTimeout = setTimeout(() => checkRecipientFriendship(username), 400);
		return () => clearTimeout(recipientCheckTimeout);
	});
			...
			<div class="space-y-2">
				...
				{#if transferType === 'CASH'}
					<p class="text-muted-foreground text-xs">
						Minimum: $10.00 per transfer &middot; {#if recipientIsFriend}<span class="text-success">fee-free (friends)</span>{:else}{(CASH_TRANSFER_FEE_RATE * 100).toFixed(0)}% fee{/if}
					</p>
						...
						<div class="flex items-center justify-between">
							<span class="text-muted-foreground text-xs">
								{#if recipientIsFriend}
									Transfer fee (friends)
								{:else}
									Transfer fee ({(CASH_TRANSFER_FEE_RATE * 100).toFixed(0)}%)
								{/if}
							</span>
							<span class="text-muted-foreground text-xs">
								{recipientIsFriend ? '$0.00' : `-$${cashTransferFee.toFixed(2)}`}
							</span>
						</div>
						<div class="flex items-center justify-between">
