<script lang="ts">
	...
	import { PENDING_REQUEST_COUNT, fetchPendingRequestCount } from '$lib/stores/friend-requests';
	...
	const data = {
		navMain: [
			...
			{ title: 'Friends', url: '/friends', icon: UserGroupIcon },
							...
							<Sidebar.MenuButton>
								{#snippet child({ props }: { props: MenuButtonProps })}
									<a
										...
										class={`${props.class} h-7! ${(item.title === 'Notifications' || item.title === 'Friends') && !$USER_DATA ? 'pointer-events-none opacity-50' : ''}`}
									>
										...
										{/if}
										{#if item.title === 'Friends' && $PENDING_REQUEST_COUNT > 0 && $USER_DATA}
											<Sidebar.MenuBadge class="bg-primary text-primary-foreground">
												{$PENDING_REQUEST_COUNT > 99 ? '99+' : $PENDING_REQUEST_COUNT}
											</Sidebar.MenuBadge>
										{/if}
