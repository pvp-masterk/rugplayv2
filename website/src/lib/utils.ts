...
export const plinko_risk_options: { value: PlinkoRisk; label: string }[] = [
    { value: 'low', label: 'Low' },
    { value: 'medium', label: 'Medium' },
    { value: 'high', label: 'High' }
];

function plinkoBinomialProbabilities(rows: number): number[] {
    const probs: number[] = [];
    let coeff = 1;
...
const plinkoRiskShapes: Record<PlinkoRisk, (dist: number) => number> = {
    low: (dist) => 1 + 4 * Math.pow(dist, 3),
    medium: (dist) => 0.4 + 9 * Math.pow(dist, 4),
    high: (dist) => 0.2 + 32 * Math.pow(dist, 6)
};

/**
 * Generates the multiplier for each landing bucket (0..rows) for a given
 * row count and risk level. Uses the same 5% house edge convention as
 * mines/tower: the probability-weighted expected payout equals (1 - HOUSE_EDGE).
 */
export function calculatePlinkoMultipliers(rows: PlinkoRows, risk: PlinkoRisk): number[] {
    const HOUSE_EDGE = 0.05;
    const probs = plinkoBinomialProbabilities(rows);
    const center = rows / 2;
    const shape = plinkoRiskShapes[risk];

    const raw = probs.map((_, k) => {
        const dist = Math.abs(k - center) / center;
        return shape(dist);
    });

    let ev = 0;
    for (let k = 0; k <= rows; k++) ev += probs[k] * raw[k];

    const target = 1 - HOUSE_EDGE;
    const scale = ev > 0 ? target / ev : 1;

    return raw.map((x) => Math.max(0.2, Number((x * scale).toFixed(2))));
}
