...
import { validateBetAmount, calculatePlinkoMultipliers, plinko_row_options, type PlinkoRisk, type PlinkoRows } from '$lib/utils';
...
export const POST: RequestHandler = async ({ request }) => {
    ...
    try {
        const { amount, rows, risk }: PlinkoRequest = await request.json();

        if (!plinko_row_options.includes(rows)) {
            return json({ error: 'Invalid row count' }, { status: 400 });
        }
        ...
        const result = await db.transaction(async (tx) => {
            ...
            // Simulate the ball dropping through each row of pegs. At each row
            // the ball bounces left (0) or right (1) with equal probability,
            // using the same crypto-secure RNG as the other arcade games.
            const path: number[] = [];
            let bucket = 0;
            for (let i = 0; i < rows; i++) {
                const bounce = randomInt(0, 2);
                path.push(bounce);
                bucket += bounce;
            }

            const multipliers = calculatePlinkoMultipliers(rows, risk);
        ...
        await publishArcadeActivity(userId, result.won ? result.payout : result.amountWagered, result.won, 'plinko', 2200);
